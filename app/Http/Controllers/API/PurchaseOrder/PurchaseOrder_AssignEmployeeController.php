    ...
    public function assign_employee(Request $request)
    {
        ...
        try {
            ...
            foreach ($filteredProductDetails as $productDetailData) {
                ...
                $remainingQuantity = $productDetail->quantity - $totalDeliveredQuantity;

                if ($productDetailData['quantity'] > $remainingQuantity) {
                    $exceedingProducts[] = [
                        'product_name' => $productDetail->product->product_name,
                        'requested_quantity' => $productDetailData['quantity'],
            ...
            if (!empty($exceedingProducts)) {
                ...
                return response()->json([
                    'error' => 'Product quantities exceed available stock or remaining purchase order quantity',
                    'exceeding_products' => $exceedingProducts // Provide detailed information
                ], 400);
            }
